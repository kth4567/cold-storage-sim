...
STATIC_AABBS = [
    ...
    (8.2, 8.4, -4.6, 5.0), (18.8, 19.0, -4.6, 5.0),            # 机房西/东墙
    (8.2, 19.0, -4.6, -4.4),                                   # 机房南墙
    (8.2, 15.6, 4.8, 5.0), (18.6, 19.0, 4.8, 5.0),             # 机房北墙(门洞 15.6~18.6)
    (9.35, 11.65, -3.25, 3.25),                                # 低压级撬块 VBU330
    (12.65, 15.15, -3.65, 3.65),                               # 高压级撬块 VHU-3640
    (18.3, 18.8, 0.4, 4.0),                                    # 机组控制柜排
    (17.25, 18.55, -3.25, -1.95),                              # 立式贮液器
...
class ColdRoomWorld:
    ...
    def _build_xml(self) -> str:
        ...
        # 制冷机房 (按真实工程图纸: 机组在独立机房内, 蒸发式冷凝器在机房屋顶)
        # 外壳 x 8.2~19.0, z -4.6~5.0, 高 6m; 卷帘门在北墙 x 15.6~18.6
        s.append(_box("mr_wall_w", (8.3, 3.0, 0.2), (0.1, 3.0, 4.8)))
        s.append(_box("mr_wall_e", (18.9, 3.0, 0.2), (0.1, 3.0, 4.8)))
        s.append(_box("mr_wall_s", (13.6, 3.0, -4.5), (5.4, 3.0, 0.1)))
        s.append(_box("mr_wall_nw", (11.9, 3.0, 4.9), (3.7, 3.0, 0.1)))
        s.append(_box("mr_wall_ne", (18.8, 3.0, 4.9), (0.2, 3.0, 0.1)))
        s.append(_box("mr_lintel", (17.1, 4.9, 4.9), (1.5, 1.1, 0.1)))       # 门楣 y3.8~6
        s.append(_box("mr_roof", (13.6, 6.09, 0.2), (5.5, 0.09, 4.9)))
        s.append(_box("equip_lp", (10.5, 1.6, 0.0), (1.15, 1.6, 3.25)))      # 低压级 VBU330
        s.append(_box("equip_hp", (13.9, 1.75, 0.0), (1.25, 1.75, 3.65)))    # 高压级 VHU-3640
        s.append(_box("equip_cab", (18.55, 1.05, 2.2), (0.25, 1.05, 1.7)))   # 控制柜排
        s.append(_box("equip_cond", (13.6, 8.6, 0.2), (2.75, 2.45, 1.85)))   # 屋顶蒸发式冷凝器
        s.append('<geom name="receiver" type="cylinder" pos="17.9 1.8 -2.6" '
                 'size="0.65 1.3" euler="1.5708 0 0"/>')                     # 热虹吸贮液器
...
<mujoco model="coldroom">
  ...
  <actuator>
    ...
    def set_player(self, pos):
        # 碰撞球中心跟随相机高度(眼高-0.75); 上限须容纳机房屋面(眼高约7.9)
        y = min(max(pos[1] - 0.75, 0.4), 8.6)
